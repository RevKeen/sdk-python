# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata


class IntegrationsActivateRequestCredentials(UniversalBaseModel):
    api_key: typing_extensions.Annotated[str, FieldMetadata(alias="apiKey")] = pydantic.Field(alias="apiKey")
    base_url: typing_extensions.Annotated[str, FieldMetadata(alias="baseUrl")] = pydantic.Field(alias="baseUrl")
    app_name: typing_extensions.Annotated[typing.Optional[str], FieldMetadata(alias="appName")] = pydantic.Field(
        alias="appName", default=None
    )
    app_email: typing_extensions.Annotated[typing.Optional[str], FieldMetadata(alias="appEmail")] = pydantic.Field(
        alias="appEmail", default=None
    )

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
